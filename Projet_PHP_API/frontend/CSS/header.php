<?php
// header.php
?>

<header>
    <nav>
        <ul>
            <li><a href="ListeJoueur.php"><button class="nav-button">Liste des joueurs</button></a></li>
            <li><a href="ListeMatch.php"><button class="nav-button">Liste des matchs</button></a></li>
            <li><a href="Statistiques.php"><button class="nav-button">Statistiques</button></a></li>
            <li><a href="?deconnexion=true"><button type="submit" name="deconnexion" class="logout-button">Se déconnecter</button></a></li>

            <!-- Ajouter d'autres liens si nécessaire -->
        </ul>
    </nav>
</header>
